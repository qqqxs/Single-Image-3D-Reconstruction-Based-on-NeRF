...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_ray_batch', type=int, default=64,
    ...
    opt.cuda_ray = True
    optDict = opt.__dict__  # 将opt转换为字典方便操作
    opt.workspace = os.path.join('results', opt.workspace)
    if opt.workspace is not None:
        os.makedirs(opt.workspace, exist_ok=True)  # 创建results文件夹存放结果
    ...
    print(opt)
    seed_everything(opt.seed)  # 定义种子

    # load depth network (from run_monodepth.py)
    net_w = net_h = 384
    ...
    if opt.text is None:
        ...
        # caption = 'a cute little dragon with purple wings and pink eyes'
        # caption = 'a teddy bear with a black bow on its neck'
        # caption = 'neuschwanstein castle, bavaria, germany'
        caption = 'back to the future delorean car'
    ...
    with torch.no_grad():
        ...
        depth_prediction = torch.nn.functional.interpolate(
            ...
            mode="bicubic",
            align_corners=True,
        )  # [1, 1, 512, 512] [80~150]  (batch_size, channels, height, width)
        DPT.util.io.write_depth_name(os.path.join(opt.workspace, opt.text.replace(" ", "_") + '_depth'),
    ...
    if opt.test:
        ...
        if opt.refine:
            ...
            test_loader = NeRFDataset(opt, device=device, type='test', H=opt.H, W=opt.W, size=64).dataloader()
            trainer.test(mv_loader, save_path=os.path.join(opt.workspace, 'mvimg'), write_image=True, write_video=True)
